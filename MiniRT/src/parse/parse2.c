...
/*   Updated: 2026/03/06 17:58:52 by gkankia          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
params, rt->file_line));
	if (ft_arrlen(params) != 4)
		return (parse_error("Wrong number of camera parameters", \
params, rt->file_line));
	rt->camera = ft_calloc(sizeof(t_camera), 1);
	if (!rt->camera)
		return (parse_error("Camera memory allocation failed", \
params, rt->file_line));
...
bool	parse_resolution(t_minirt *rt, char *line)
{
	...
	if (ft_arrlen(params) != 3)
		return (parse_error("Invalid resolution parameters", \
params, rt->file_line));
	while (params[++i])
	{
...
params, rt->file_line));
	...
	if (!light)
		return (parse_error("Light memory allocation failed", \
params, rt->file_line));
	while (params[++i])
	{
		if (i == 1 && !parse_vector(params[i], &light->position))
			return (parse_error_ptr("Invalid light position", NULL, \
params, rt->file_line));
		if (i == 2 && !parse_double(params[i], &light->brightness))
			return (parse_error_ptr("Invalid light brightness", NULL, \
params, rt->file_line));
		if (i == 3 && !parse_color(params[i], &light->color))
			return (parse_error_ptr("Invalid light color", NULL, \
params, rt->file_line));
	}
	free_arr(params);
	return (true);
